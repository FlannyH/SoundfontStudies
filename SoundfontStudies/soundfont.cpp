...
namespace Flan {
    ...
    bool Soundfont::from_file(std::string path) {
        ...
        while (1) {
            ...
            new_sample.data = (i16*)malloc(new_sample.length * sizeof(u16) * new_sample.n_channels);
            if (new_sample.data == nullptr) {
                __noop;
            }
    ...
    Preset Soundfont::get_preset_from_index(size_t index) {
        ...
        for (uint16_t preset_zone_index = preset_zone_start; preset_zone_index < zone_end; preset_zone_index++) {
            ...
            for (uint16_t instrument_index = instrument_start; instrument_index < instrument_end; instrument_index++) {
                ...
                {
                    final_zone_generator_values["overridingRootKey"].s_amount = sample_headers[final_zone_generator_values["sampleID"].u_amount].original_key;
                }
                ...
                Zone new_zone_to_add{
                    ...
                    sample_headers[final_zone_generator_values["sampleID"].u_amount].original_key - final_zone_generator_values["overridingRootKey"].s_amount,
    ...
    void Soundfont::clear()
    {
        ...
        {
            free(sample.second.data);
        }
        samples.clear();
        presets.clear();

        // Clear other pointers
        void* pointers_to_clear[] = { preset_headers, preset_bags, preset_mods, preset_gens, instruments, instr_bags, instr_mods, instr_gens, sample_headers };
        for (auto pointer : pointers_to_clear)
            free(pointer);

        // Reset struct to zero
        memset(&preset_headers, 0, sizeof(Soundfont) - offsetof(Soundfont, preset_headers));
    }
    ;
